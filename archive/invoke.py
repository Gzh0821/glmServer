...
TEMPLATE = """
以下简短的提示用于指导Al绘画模型创建图像。它们包括外观和背景描述、颜色、图片质量，以及图像的主题和风格等各种细节。下面是三个示例:
...
上面是三个提示的示例，仿照这些示例，使用英文，简略的写一段描写如下要素的提示，不要超过60词："""
...
baseSDXL = StableDiffusionXLPipeline.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0",
                                         torch_dtype=torch.float16,
                                         use_safetensors=True,
                                         variant="fp16",
                                         add_watermarker=False)
baseSDXL.enable_model_cpu_offload()

baseSDXL.enable_vae_slicing()
